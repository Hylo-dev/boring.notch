...
class MusicManager: ObservableObject {
    ...
    func updateAlbumArt(newAlbumArt: NSImage) {
        ...
        let smallSize = NSSize(width: 200, height: 200)
        
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let optimizedImage: NSImage
            if newAlbumArt.size.width > 300 {
                optimizedImage = newAlbumArt.resized(to: smallSize) ?? newAlbumArt
                
            } else {
                optimizedImage = newAlbumArt
            }
                    
            Task { @MainActor in
                withAnimation(.smooth) {
                    self?.albumArt = optimizedImage
